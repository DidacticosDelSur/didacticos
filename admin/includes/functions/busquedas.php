...
function listar_busquedas($db,$t,$url_site){
  ...
  $sql = "SELECT bc.busqueda,IF(isNull(c.id), 'Usuario no logueado', IF((bc.es_vendedor = 1), CONCAT(v.nombre,' ',v.apellido,' (',c.nombre,' ',c.apellido,')'), CONCAT(c.nombre,' ',c.apellido))) as usuario, bc.resultado, bc.link, DATE_FORMAT(bc.fecha, '%d/%m/%Y %H:%i') as fecha 
